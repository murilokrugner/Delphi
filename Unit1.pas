  ...
  if ComboBox1.ItemIndex = 0 then
    ...
    if ComboBox1.ItemIndex = 1 then
      begin
        ...
        ADOQuery1.SQL.Add('select c7_num as NUMERO_DO_PEDIDO, format(convert(date, C7_EMISSAO, 103),'+chr(39)+date+chr(39)+') as DATA_DE_EMISSÃO,'
        +'C7_FORNECE AS CODIGO_DO_FORNECEDOR, C7_ITEM AS ITEM, C7_DESCRI AS DESCRIÇÃO_DO_PRODUTO, C7_UM AS UNIDADE, C7_QUANT AS QUANTIDADE,'
        +'C7_QUJE AS QUANTIDADE_JA_ENTREGUE, format(convert(date, c7_datprf, 103),'+chr(39)+date+chr(39)+') as DATA_DE_ENTREGA FROM SC7010 JOIN sa2010 on a2_cod = c7_fornece where a2_nome = '+chr(39)+search+chr(39) +'and c7_filial = 01 order by c7_num desc');
        ADOQuery1.Open
      end
    else
...
begin
  ...
  Unit3.Form3.ADOQuery2.SQL.Add('select top 1 a2_nome, a2_end, a2_cgc as CNPJ, a2_pricom, a2_ultcom, a2_tel, a2_est from sa2010 JOIN sc7010 on c7_fornece = a2_cod where c7_num = '+chr(39)+select+chr(39));
  ...
  Unit3.Form3.ADOQuery7.SQL.Add('select TOP 1 E4_DESCRI from SE4010 JOIN sc7010 on c7_cond = e4_codigo where E4_FILIAL = 01 AND c7_num ='+chr(39)+select+chr(39));
  Unit3.Form3.ADOQuery10.SQL.Add('select sum(c7_baseicm) as BASE_ICMS, c7_picm as ALIQUOTA_ICMS, (c7_picm / 100 * sum(c7_baseicm)) as TOTAL_ICMS from sc7010 where c7_num = '+chr(39)+select+chr(39)+ 'GROUP BY C7_PICM');
  Unit3.Form3.ADOQuery11.SQL.Add('select sum(c7_baseipi) as BASE_IPI, c7_ipi as ALIQUOTA_IPI, (c7_ipi / 100 * sum(c7_baseipi)) as TOTAL_IPI from sc7010 where c7_num ='+chr(39)+select+chr(39)+ 'GROUP BY C7_IPI');
  ...
  totalPedido := (totalProdutos + totalFrete + totalDespesa + totalSeguro + totalIpi) - totalDesconto;
  ...
  cnpjConvert := copy(cnpj,1,2)+'.'+copy(cnpj,3,3)+'.'+copy(cnpj,6,3)+'/'+copy(cnpj,9,4)+'-'+copy(cnpj,13,2);
