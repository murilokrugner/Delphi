...
type
  TForm1 = class(TForm)
    ...
    ADOQuery1DATA_DE_EMISSÃO: TStringField;
...
begin
  ...
  if ComboBox1.ItemIndex = 0 then
    begin
      ADOQuery1.SQL.Add('SELECT c7_num as NUMERO_DO_PEDIDO, C7_EMISSAO AS DATA_DE_EMISSÃO, C7_FORNECE AS CODIGO_DO_FORNECEDOR, C7_ITEM AS ITEM,'+
      'C7_DESCRI AS DESCRIÇÃO_DO_PRODUTO, C7_UM AS UNIDADE, C7_QUANT AS QUANTIDADE, C7_QUJE AS QUANTIDADE_JA_ENTREGUE, C7_DATPRF AS DATA_DE_ENTREGA FROM SC7010 WHERE C7_NUM LIKE'+chr(39)+search+chr(39));
      ADOQuery1.Open
    end
    else
    if ComboBox1.ItemIndex = 1 then
      begin
        ADOQuery1.SQL.Add('select c7_num as NUMERO_DO_PEDIDO, C7_EMISSAO AS DATA_DE_EMISSÃO, C7_FORNECE AS CODIGO_DO_FORNECEDOR, C7_ITEM AS ITEM, C7_DESCRI AS DESCRIÇÃO_DO_PRODUTO, C7_UM AS UNIDADE,'+
        'C7_QUANT AS QUANTIDADE, C7_QUJE AS QUANTIDADE_JA_ENTREGUE, C7_DATPRF AS DATA_DE_ENTREGA FROM SC7010 JOIN sa2010 on a2_cod = c7_fornece where a2_nreduz = ' + chr(39)+search+chr(39) +'and c7_filial = 01 order by c7_num desc');
        ADOQuery1.Open
      end
    else
    if ComboBox1.ItemIndex = 2 then
      begin
        ADOQuery1.SQL.Add
          ('select c7_num as NUMERO_DO_PEDIDO, C7_EMISSAO AS DATA_DE_EMISSÃO, C7_FORNECE AS CODIGO_DO_FORNECEDOR, C7_ITEM AS ITEM, C7_DESCRI AS DESCRIÇÃO_DO_PRODUTO,'+
          'C7_UM AS UNIDADE, C7_QUANT AS QUANTIDADE, C7_QUJE AS QUANTIDADE_JA_ENTREGUE, C7_DATPRF AS DATA_DE_ENTREGA FROM SC7010 JOIN sa2010 on a2_cod = c7_fornece where a2_cgc = ' + chr(39)+search+chr(39) +'and c7_filial = 01 order by c7_num desc');
        ADOQuery1.Open
      end
    else
    if ComboBox1.ItemIndex = 3 then
      begin
        ADOQuery1.SQL.Add
          ('select c7_num as NUMERO_DO_PEDIDO, C7_EMISSAO AS DATA_DE_EMISSÃO, C7_FORNECE AS CODIGO_DO_FORNECEDOR, C7_ITEM AS ITEM, C7_DESCRI AS DESCRIÇÃO_DO_PRODUTO,'+
          'C7_UM AS UNIDADE, C7_QUANT AS QUANTIDADE, C7_QUJE AS QUANTIDADE_JA_ENTREGUE, C7_DATPRF AS DATA_DE_ENTREGA FROM SC7010 where c7_datprf =' + chr(39)+search+chr(39));
        ADOQuery1.Open
      end
  end;
...
begin
  ...
  ADOQuery1.SQL.Add('select c7_num as NUMERO_DO_PEDIDO, C7_EMISSAO AS DATA_DE_EMISSÃO, C7_FORNECE AS CODIGO_DO_FORNECEDOR, C7_ITEM AS ITEM, C7_DESCRI AS DESCRIÇÃO_DO_PRODUTO,'+
  'C7_UM AS UNIDADE, C7_QUANT AS QUANTIDADE, C7_QUJE AS QUANTIDADE_JA_ENTREGUE, C7_DATPRF AS DATA_DE_ENTREGA FROM SC7010 where C7_FILIAL = 01 order by c7_num desc');
  ADOQuery1.Open;
end;
...
begin
  ...
  //converter a data
  dataConvert := copy(dataBanco,7,2)+'/'+copy(dataBanco,5,2)+'/'+copy(dataBanco,1,4);
